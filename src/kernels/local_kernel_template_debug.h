...
#define CORE_LOCAL_COMPUTE() \
		if (ridx + W < gidx + m-1 || ridx - W > gidx + m-1) { \
			...
			p[m] = h[m-1]; \
			diag_idx = ((gidx + m-1+ridx)%total_shm)<<5;\
...
__global__ void gasal_local_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, gasal_res_t *device_res_second, uint4 *packed_tb_matrices, int n_tasks, uint32_t max_query_len, short2 *global_inter_row, int stretch)
{
	...
	int real_warp_id = threadIdx.x % 32;
	...
	//int iter_num;
	int W = 751;
	int band_start, band_end, completed_band;
	...
	short2* global_inter_row_old = (short2*)(global_inter_col+max_query_len*(blockDim.x/8)*gridDim.x);
	short2* swap;
	int zdrop = 400;
	bool zdropped = false;
	...
	int32_t diag_idx;
	//unsigned const mask = (0xffffffff) << (tx % 32);
	unsigned same_threads = __match_any_sync(0xffffffff, warp_num);
	
	bool active;
	int stretch_start, stretch_end;
	for (i = job_start_idx; i < job_end_idx; i++) {
		...
		for (j = 0; j < job_per_query; j++) {
			...
			if ((l) < max_query_len) {
				temp = -(_cudaGapOE + (_cudaGapExtend*(l)));
				global_inter_row[warp_num*max_query_len + l] =  l <= W? make_short2(temp, temp-_cudaGapOE):initHD;	
		...
		for (j = 0; j < job_per_query; j++) {
			...
			if ((l) < max_query_len) {
				temp = -(_cudaGapOE + (_cudaGapExtend*(l)));
				global_inter_row_old[warp_num*max_query_len + l] =  l <= W? make_short2(temp, temp-_cudaGapOE):initHD;
				//inter_row[warp_block_id*512 + l] = global_inter_row[warp_num*max_query_len + l];	
			}
		}
		for (j = 0; j < job_per_query; j++) {
			...
			if ((l) < max_query_len) {
				temp = -(_cudaGapOE + (_cudaGapExtend*(l)));
				global_inter_col[warp_num*max_query_len + l] =  l <= W? make_short2(temp, temp-_cudaGapOE):initHD;
		...
		while (j < total_diags) {
			...
			*/
			...
			while (completed_band <= band_end) {
				...
				if (active) {
					...
					if (ridx>0) {
						HD = global_inter_row[warp_num*max_query_len + ridx-1];
						p[1] = HD.x;
					} else {
						if (gidx==0) p[1] = 0;
						else p[1] = gidx-1 <=W? -(_cudaGapOE + (_cudaGapExtend*(gidx-1))):MINUS_INF2;
					}
				...
				for (y = 0; y < stretch; y++) {
					...
					if (active && stretch_start <= band_query && band_query <= stretch_end) {
						...
						for (k = 28; k >= 0 && ridx < read_len; k -= 4) {
							...
							HD = global_inter_row_old[warp_num*max_query_len + ridx];
							//HD = global_inter_row[warp_num*max_query_len + ridx];
							//p[1] = HD.x;
							h[0] = HD.x;
							e = HD.y;
							
							if (band_target == stretch_start || band_target == stretch_end) {
								#pragma unroll 8
			...
			*/
			...
			//check each diag's max value
			l = read_len+ref_len-1;
			for (k = j*packed_len; k < j*packed_len+total_shm - packed_len; k++) {
				if (k < l) {
					m = k%(total_shm);
		...
		//check each diag's max value
		if (!zdropped) {
			diag_idx = (j*packed_len)&15;
			for (k = j*packed_len; k < read_len+ref_len-1; k++) {
				m = k%(total_shm);
				temp = __reduce_max_sync(same_threads, diag_maxHH[(m<<5)+real_warp_id]);
				if ((temp>>16) > maxHH) {				
					maxHH = temp>>16;
